...
def platforms(request):
    ...
    if 'admin' in request.session:
        if request.method == 'POST':
            ...
            mem3.save()
        return render(request, 'administrator/platforms.html',{'s':s})
    else:
        return redirect('admin_login')
...
def usercreate(request):
        ...
        if request.method == 'POST':
            ...
            platformid = request.POST['platform__id']
            level = request.POST['level']
            email = request.POST['email']
...
def userreg(request):
    ...
    return render(request,'user/userreg.html',{'v':v,'c':c})
...
def platform_delete(request,id):
        ...
        abc=Addnewplatform.objects.get(id=id)
        abc.delete()
        return redirect('mainplatform')
def platformedit(request):
    mem4=Addnewplatform.objects.filter(id=2)
    return render(request,"Administrator/platform_edit.html",{'mem4':mem4,})
...
def platform_edit(request):
   ...
   if 'admin' in request.session:
        if request.method == 'POST':
            s=Addnewplatform.objects.all
            mem4 = Addnewplatform()
            mem4.platformname= request.POST['platform_name']
            mem4.uploadthumbnail = request.FILES['upload_img']
            mem4.description = request.POST['platform_description']
            mem4.tutorial_discription = request.POST['tutorial_discription']
            mem4.tutorial_video= request.FILES['tutorial_video']
    
            mem4.save()
        return redirect('platform_edit',{'s':s})